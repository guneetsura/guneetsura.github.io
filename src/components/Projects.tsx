...
const Projects: React.FC = () => {
  ...
  return (
    <section id="projects" className="section">
      ...
      <div className="grid sm:grid-cols-2 gap-6">
        {projects.map((project, index) => (
          <motion.div
            ...
            className={`card p-6 flex flex-col ${project.featured ? 'sm:col-span-1' : ''}`}
          >
            {project.award && (
              <div className="flex items-center gap-1.5 text-xs text-[var(--accent)] mb-3">
                <Trophy size={14} />
            ...
            <div className="flex gap-5 mt-auto pt-4 border-t border-[var(--border)]">
              {project.github && (
                <a
                  ...
                  className="flex items-center gap-1.5 text-sm text-[var(--text-faint)] hover:text-[var(--text)] transition-colors"
                >
                  <Github size={16} />
                  Code
                </a>
      ...
      >
        <a
          href="https://github.com/guneetsura"
          target="_blank"
          rel="noopener noreferrer"
          className="link-underline text-sm text-[var(--text-muted)]"
        >
          See more on GitHub →
        </a>
      </motion.div>
